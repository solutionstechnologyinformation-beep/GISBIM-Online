...
try:
    ...
    from . import ibge_data, mapbiomas_data
    from .datum_converter import convert_datum, get_available_datums
except ImportError:
    from upload_simple import process_upload
    from spatial import (
        dms_to_dd, dd_to_dms, dd_to_utm, utm_to_dd,
...
def health():
    ...
    return jsonify({
        'message': 'GIS Online está funcionando',
        'status': 'healthy',
        'version': '3.0.0',
        'type': 'Plataforma de Análise Geoespacial'
    }), 200
...
@app.route('/layers/ibge/municipios', methods=['GET'])
def get_ibge_municipios():
    """Retorna dados de municípios do IBGE em GeoJSON."""
    try:
        data = ibge_data.get_municipios_geojson()
        if data:
            return jsonify(data), 200
        else:
            return jsonify({'error': 'Erro ao obter dados do IBGE'}), 500
    except Exception as e:
        return jsonify({'error': str(e)}), 500


@app.route('/layers/ibge/estados', methods=['GET'])
def get_ibge_estados():
    """Retorna dados de estados do IBGE em GeoJSON."""
    try:
        data = ibge_data.get_estados_geojson()
        if data:
            return jsonify(data), 200
        else:
            return jsonify({'error': 'Erro ao obter dados do IBGE'}), 500
...
@app.route('/layers/mapbiomas/info', methods=['GET'])
def get_mapbiomas_info():
    """Retorna informações sobre camadas do MapBiomas."""
    try:
        layers = mapbiomas_data.get_all_layers()
        biomas = mapbiomas_data.get_biomas_list()
        classes = mapbiomas_data.get_classification_colors()
...
@app.route('/convert/datum', methods=['POST'])
def convert_datum_route():
    """Converte coordenadas entre datums geodésicos."""
    try:
        data = request.json
        if not data:
            return jsonify({'error': 'Nenhum dado JSON fornecido'}), 400
        
        required_fields = ['latitude', 'longitude', 'src_datum', 'dst_datum']
        for field in required_fields:
            if field not in data:
                return jsonify({'error': f'Campo obrigatório faltando: {field}'}), 400
        ...
        src_datum = str(data['src_datum']).strip()
        dst_datum = str(data['dst_datum']).strip()
        
        available_datums = get_available_datums()
        if src_datum not in available_datums:
            return jsonify({'error': f'Datum de origem inválido: {src_datum}'}), 400
        if dst_datum not in available_datums:
            return jsonify({'error': f'Datum de destino inválido: {dst_datum}'}), 400
        
        result = convert_datum(latitude, longitude, src_datum, dst_datum)
        return jsonify(result), 200
    
    except Exception as e:
        return jsonify({'error': f'Erro ao converter datum: {str(e)}'}), 500


@app.route('/datums', methods=['GET'])
def list_datums():
    """Lista todos os datums disponíveis."""
    try:
        datums = get_available_datums()
        return jsonify({
            'datums': datums,
            'total': len(datums)
        }), 200
